...
//    private ClassComparator classComparator = new ClassComparator();
    ...
    public String[] orderClasses(String[] param0) {
        ...
        //              add dependencies in front of dependency
        //      add class after all dependencies
        
        String[] mySchedule = new String[classPrerequisites.size()];
        for (Entry<String, List<String>> map : classPrerequisites.entrySet()) {
            for (String prerequisite : map.getValue()) {
                addClassToSchedule(prerequisite);
            }
            addClassToSchedule(prerequisite);
        }
...
//        TreeSet<String> schedule = new TreeSet<>(new ClassComparator());
        ...
        for (Entry<String, List<String>> map : classPrerequisites.entrySet()) {
            try {
                addToSchedule(map.getKey(), map.getValue());
...
//        return schedule;
    ...
    private void addToSchedule(String className, List<String> prerequisites) {
        if (!schedule.contains(className)) {
            schedule.add(className);
            return;
        }
        
//        String prerequisite = prerequisites.remove(0);
//
//        for (String prerequisite : prerequisites) {
//            addToSchedule(prerequisite, classPrerequisites.get(prerequisite));
//        }
    }
